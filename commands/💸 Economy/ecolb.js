    ...
    try {
      ...
      ensure_economy_user(client, message.guild.id, user.id)
        ...
        const queueEmbed = await message.reply(
          {embeds: [embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))]}
          ...
          try {
            if (reaction.emoji?.name === reactionemojis[1] || reaction.emoji?.id  === reactionemojis[1]) {
              if (currentPage < embeds.length - 1) {
                currentPage++;
                queueEmbed.edit({embeds: [embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))]});
              } else {
                currentPage = 0
                queueEmbed.edit({ embeds: [embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))]});
              }
