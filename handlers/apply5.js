...
module.exports = client => {
  ...
  let applyname = require("path").parse(__filename).name;
  let applytickets = `applytickets${String(applyname).replace("apply", "").length > 0 ? String(applyname).replace("apply", "") : "1"}`; 
  let ticketsetupapply = `ticket-setup-apply-${String(applyname).replace("apply", "").length > 0 ? String(applyname).replace("apply", "") : "1"}`; 
        ...
        try {
          ...
          if (interaction?.customId == "Apply_accept") {
            ...
            var approve = new Discord.MessageEmbed().setFooter(es.footertext, es.footericon)
              ...
              .setFooter("By  |  " + interaction?.user.tag, interaction?.user.displayAvatarURL({
                dynamic: true
              }))
