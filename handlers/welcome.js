...
module.exports = client => {
  ...
  client.on("ready", async () => {
    ...
    client.invitations = {};
    await asyncForEach([...client.guilds.cache.values()], async (guild) => {
    ...
    async function welcome(member) {
      ...
      if (!perm) {
        ...
        if (guildInvites && oldGuildInvites) {
          ...
          let inviteUsed = guildInvites.find((i) => oldGuildInvites.get(i?.code) && (oldGuildInvites.get(i?.code).uses ? oldGuildInvites.get(i?.code).uses : "Infinite") < i?.uses);
      ...
      const inviter = invite && invite.inviter ? await resolveUser(invite.inviter.id) : null;
      ...
      if (inviter) {
        ...
        // If the Inviter previously invited the member, and he left but now rejoined
