...
module.exports = client => {
    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if(!client.setups.has(channel.id)){
            return interaction?.reply({content: ":x: This channel is not a Ticket", ephemeral: true})
        }
                            ...
                            if (element.id == user.id) {
                                if (!element.allow.includes("VIEW_CHANNEL")) {
                                    ...
                                    channel.permissionOverwrites.edit(user.id, {
                                            ...
                                            channel.send({
                                                content: `<@${buttonuser.id}>`,
                                                embeds: [new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                                                    .setFooter(es.footertext, es.footericon)
            ...
            channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription(ticket.claim.messageClaim.replace(/\{claimer\}/ig, `${member.user}`).replace(/\{user\}/ig, `<@${data.user}>`))
                ]
            }).catch(e => {console.log(e.stack ? String(e.stack).grey : String(e).grey)});
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            client.menuticket.ensure(guild.id, {
                messageId: "",
                channelId: "",
            ...
            let settings = client.menuticket.get(guild.id);
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                ...
                try {
                    ...
                    if(cat){
                        ...
                        if(channel.parent.children.size < 50){
                            await ch.setParent(String(channel.parent.id), {lockPermissions: false}).catch(() => {});
                        }
                        }
                    }
                } catch (e){
                ...
                var ticketembed = new MessageEmbed()
                    ...
                    .setDescription(replyMsg.replace(/\{user\}/igu, `${user}`))
    ...
    client.on("interactionCreate", interaction => {
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            let { user, message, channelId, values, guild } = interaction;
            client.autosupport.ensure(guild.id, {
                messageId: "",
                channelId: "",
                data: [ //all menus in there
                    /*
            ...
            let settings = client.autosupport.get(guild.id);
            let es = client.settings.get(guild.id, "embed")
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
                let index = settings.data.findIndex(v => v.value == values[0]);
                if(index < 0) {
