...
module.exports = (client) => {
  ...
  let filename = require("path").parse(__filename).name;
  let systempath = `ticketsystem${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"}`; 
  let ticketspath = `tickets${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"}`; 
  let idpath = `ticketid${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"}`; 
  let tickettypepath = `ticket-setup-${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"}`; 
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    let channelname = `ticket-${user.username}`.replace(/\s/igu, "-").substr(0, 31);
    let optionsData = {
    ...
    guild.channels.create(channelname.substr(0, 31), optionsData).then(async ch => {
      ...
      if(ticket.claim.enabled){
        var claimEmbed = new MessageEmbed()
          .setColor("ORANGE").setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setAuthor(`A Staff Member will claim the Ticket soon!`, "https://cdn.discordapp.com/emojis/833101350623117342.gif?size=44", "https://discord.gg/milrato")
