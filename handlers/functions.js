...
function edit_msg(client, guild, thedb){
...
function send_roster(client, guild, thedb){
  send_roster_msg(client, guild, thedb)
}
...
async function edit_Roster_msg(client, guild, the_roster_db) {
  try{
    ...
    let message = await channel.messages.fetch(data.rostermessage).catch((e)=>{
      console.log("ERROR CATCHING, EDIT_MSG", e.stack ? String(e.stack).grey.bgRed : String(e).grey.bgRed)
    });
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, "showallroles") || memberarray.length < 20)
            ...
            try {
              rosterembed.addField(`\u200b`, role.members.map(this_Code_is_by_Tomato_6966 => this_Code_is_by_Tomato_6966).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, "inline"))
            } catch (e) {
              console.log(e.stack ? String(e.stack).grey : String(e).grey)
            }
        }
