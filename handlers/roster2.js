...
const {
    edit_msg2, databasing
} = require(`${process.cwd()}/handlers/functions`);

module.exports = (client) => {
/*MOVED TO roster.js
    var CronJob = require('cron').CronJob;
    client.Jobroster2 = new CronJob('0 10 * * * *', function() {
        client.guilds.cache.map(guild => {
            client.roster2.ensure(guild.id, {
            ...
            edit_msg2(client, guild)
        });
    }, null, true, 'America/Los_Angeles');
    client.Jobroster2.start();
        ...
        try {
            ...
            if (rolechanged) {
                ...
                if (roleadded.length > 0) {
                    client.roster2.ensure(newMember.guild.id, {
                        rosterchannel: "notvalid",
                        rostermessage: "",
                        rosterroles: [],
                      })
                    let rosterroles = client.roster2.get(newMember.guild.id, "rosterroles");
                    ...
                    for (let i = 0; i < rosterroles.length; i++) {
                        ...
                        if (roleadded.includes(role.id)) {
                            if (oldStateMap.get("SOMETHING")) {
                                ...
                                edit_msg2(client, newMember.guild);
                                setTimeout(() => {
