...
module.exports = async client => {
  ...
  client.Joblivelog = new CronJob('0 */7 * * * *', async function() {
    ...
    console.log(` [TWITCH] :: Checking Accounts - ${moment().format("LLLL")}`.magenta)
  ...
  function getStreams(guild){
    ...
    if(!tempData.channels) return console.log(" [TWITCH] :: NO TWITCH DATA (TW-CHANNELS)".magenta)
    ...
    tempData.channels.map(async function (chan, i) {
        ...
        if(!StreamData) return console.log(" [TWITCH] :: No Stream Data".magenta)
          ...
          if (chan.twitch_stream_id != StreamData.id) {
            ...
            ch.send({content: `${channelObj.message.length > 0 ? channelObj.message.substr(0, 2000) : "\u200b"}`, embeds: [embed]}).then(msg => {
                ...
                if(tempData.roleID_PING && tempData.roleID_PING.length > 2){
                  ch.send({content: `<@&${tempData.roleID_PING}>`}).then(msg=>msg.delete().catch(e=>console.log(" [TWITCH] :: Prevented delete bug".gray))).catch(e=>console.log("prevented send bug role".gray))
                }
