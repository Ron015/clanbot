...
async function create_twit(client){
      ...
      if(!userids || userids.length == 0) return console.log(` [TIKTOK] :: NO USERIDS: ${userids}`.cyan)
      //create a new TWIT
      var T = new Twit({
      ...
      console.log(String(" [TIKTOK] :: " + userids.filter(i=> i?.toLowerCase() != "impressgm").join(", ")).cyan)
      for(const user of userids){
        if(String(user).toLowerCase().includes("impressgm")) continue;
        console.log(String(" [TIKTOK] :: Checking: " + user).cyan);
        gettwit(user);
        await delay(1500);
      }
      ...
      async function gettwit(user){
        try{
          await T.get('search/tweets', { q: `from:${user}`, count: 10 }, function(err, data, response) {
            try{
              console.log(" [TIKTOK] :: DATA RECEIVED".cyan)
              if(err) return console.log(String(" [TIKTOK] :: "+err).grey)
              ...
              if(!guildid || guildid == null || guildid == undefined || guildid.length != 18) return console.log(" [TIKTOK] :: NO VALID GUILD ".cyan)
              try{
              ...
              if(twitsettings.latesttweet === tweet.id_str) return console.log(" [TIKTOK] :: LATEST TWEET".cyan)
              ...
              return console.log(String(" [TIKTOK] :: NOT RIGHT TWEET! :: " + url).cyan)
              //If retweet, do that
                ...
                client.channels.fetch(twitsettings.DISCORD_CHANNEL_ID).then(channel => {
                  channel.send(twitsettings.infomsg.replace("{Twittername}", TwitterName).replace("{url}", url)).then(msg=>{
                    console.log(` [TIKTOK] :: NOTIFICATION SENT IN ${channel.name} for ${TwitterName} with ${url}`.green)
                    //set the new latest tweet
                    client.social_log.set(guildid, tweet.id_str, "twitter.latesttweet")
                  }).catch(e=>{
                    console.log(String(" [TIKTOK] :: "+e).grey.grey)
                  })
                }).catch(err => {
                  console.log(String(" [TIKTOK] :: "+err).grey)
                })
              ...
              console.log(String(" [TIKTOK] :: "+e).grey)
            }
          })
        }catch (e){
          console.log(String(" [TIKTOK] :: "+e).grey)
